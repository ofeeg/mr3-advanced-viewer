...
impl Sandbox for Viewer {
	...
	let seventeenth_column = val!(mr3_data::determine_gr(self.data.0.15), "GrowthRate");
	let sixteenth_column = val!((self.data.1).to_string(),"Money");

	Column::with_children(vec!
			      [
				  button_column,
				  Row::with_children(vec![first_column, second_column, third_column, fourth_column,fifth_column, sixth_column, seventh_column, seventeenth_column]).into(),
